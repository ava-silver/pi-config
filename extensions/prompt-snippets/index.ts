...
interface OpenPromptSnippetsRequest {
...
function parseSnippet(filename: string, raw: string): Snippet | null {
  ...
  for (const line of match[1]?.split(/\r?\n/) ?? []) {
    ...
    if (kv?.[1] && kv[2]) meta[kv[1].toLowerCase()] = kv[2].trim().replace(/^["']|["']$/g, "");
  }
...
function updateWidget(ctx: ExtensionContext, snippets: Snippet[], enabled: Set<string>) {
...
export default function (pi: ExtensionAPI) {
  ...
  pi.events.on("prompt-snippets:open", (data) => {
    void openMenu((data as OpenPromptSnippetsRequest).ctx);
  });
